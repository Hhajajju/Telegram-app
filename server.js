...
// MongoDB URI and Telegram Bot Token from environment variables
...
app.post('/api/claim/dailyReward/:userId', async (req, res) => {
  try {
    ...
    // Update the user's balance with daily reward
    const dailyReward = 0.003;
    user.balance += dailyReward;
    user.dailyClaimTimeRemaining = now.getTime() / 1000;
    await user.save();

    res.json({
      message: `You claimed your daily reward of $${dailyReward}! Your new balance is $${user.balance.toFixed(3)}`,
      balance: user.balance.toFixed(3)
    });
  } catch (err) {
...
app.post('/api/withdraw/:userId', async (req, res) => {
  try {
    ...
    const withdrawalCooldown = 60 * 60 * 24; // 24 hours cooldown
    const lastWithdrawalTime = user.lastWithdrawal ? new Date(user.lastWithdrawal) : null;
    const elapsedTime = lastWithdrawalTime ? Math.floor((new Date() - lastWithdrawalTime) / 1000) : null;
    const remainingCooldown = withdrawalCooldown - (elapsedTime || 0);

    if (remainingCooldown > 0) {
      return res.status(400).json({ message: `Please wait ${remainingCooldown} seconds before making another withdrawal.` });
    }
    ...
    res.json({
      message: `Withdrawal of $${amount} successful! Your new balance is $${user.balance.toFixed(3)}.`,
      balance: user.balance.toFixed(3)
    });
  } catch (err) {
